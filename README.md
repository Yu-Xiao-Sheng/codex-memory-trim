...
你有没有这种感觉：Codex 用得越久，它变得越慢、越啰嗦？明明是个简单需求，它却先翻出一堆历史规矩——这个要先验证、那个要走审批，一件小事硬是做成了流程审计。更糟的是偶尔还会陷进某个循环里，反反复复尝试解决一个根本不存在的问题，token 烧了一大把，事情没办成。
...
根源在记忆。Codex 的全局记忆藏在 `~/.codex/memories` 里，随着使用悄悄增长。这些经验大部分不是手动添加的，而是每次会话结束 Codex 自己总结写入的，所有会话都可能往里添加全局记忆。经验复用本身是好事：新任务先参考历史经验，能少走很多弯路。但记忆不会自己退休。上上个项目的验收流程、只出现过一次的报错处理、早已过期的流水线号、连命令都没记下来的空会话，全都躺在记忆里，而且**每一个新任务开始时都会先被检索一遍**。
...
说到底，我是把 Codex 当自己的第二个大脑在用的。既是大脑，就得按用脑的科学方式对待：只进不出是不行的，输入太多只会越来越乱，定期减负是刚需。所以别只把它当工具来"使用"，要把它当另一个脑子来"管理"；而管理它的前提，是承认这个脑子也会犯知识诅咒。

这个项目干的就是"管理"这件事，分两步：一是减负，清掉过期和重复的条目，把啰嗦的表达压短；二是立规矩，明确告诉 Codex 什么时候该严格、什么时候该快。两步都固化成了 skill，任何一个 Codex 会话都能安全执行，不需要你手工去啃那几百 KB 的 Markdown。
